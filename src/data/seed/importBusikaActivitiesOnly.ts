/**
 * Import script for Busika Activities ONLY
 * 
...
// Busika Activities ONLY
const busikaActivitiesData = [
  {
    parkId: 'BUSIKA',
    category: 'Activities',
...
/**
 * Import Busika activities to Firestore
 */
export async function importBusikaActivities(): Promise<{
  ...
  for (const item of busikaActivitiesData) {
    try {
      ...
      try {
        const docRef = await addDoc(pricingCatalogRef, docData);
        console.log(`✅ Successfully added: ${item.itemName} (ID: ${docRef.id})`);
        results.success++;
      } catch (addError) {
        console.error(`❌❌❌ ADD DOC ERROR for ${item.itemName}:`, addError);
        throw addError; // Re-throw to be caught by outer catch
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error(`❌ Error adding ${item.itemName}:`, error, errorMessage);
      results.errors.push({
        itemName: item.itemName,
        error: errorMessage,
      });
